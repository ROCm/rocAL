...
do 
    ...
    do 
        ...
        # Set 1 augmentation tests - random variants
        ...
        ./unit_tests 8 "$caffe2_classification_path" "${output_path}Log_${rgb_name[$rgb]}_${device_name}" $width $height 69 $device $rgb 1 $display
        ./unit_tests 0 "$image_path" "${output_path}ColorJitterRandom_${rgb_name[$rgb]}_${device_name}" $width $height 70 $device $rgb 0 $display
        ./unit_tests 0 "$image_path" "${output_path}WaterRandom_${rgb_name[$rgb]}_${device_name}" $width $height 72 $device $rgb 1 $display

        # Set 1 augmentation tests - fixed variants
        ...
        ./unit_tests 0 "$image_path" "${output_path}ColorJitter_${rgb_name[$rgb]}_${device_name}" $width $height 71 $device $rgb 0 $display
        ./unit_tests 0 "$image_path" "${output_path}Water_${rgb_name[$rgb]}_${device_name}" $width $height 73 $device $rgb 0 $display
