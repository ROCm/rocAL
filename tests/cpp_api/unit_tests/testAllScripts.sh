...
do 
    ...
    do 
        ...
        # Set 2 augmentation tests - random variants
        ./unit_tests 0 "$image_path" "${output_path}ChannelPermute_${rgb_name[$rgb]}_${device_name}" $width $height 74 $device $rgb 1 $display
        ./unit_tests 6 "$caffe_classification_path" "${output_path}JpegCompressionDistortionRandom_${rgb_name[$rgb]}_${device_name}" $width $height 75 $device $rgb 1 $display
        ...
        ./unit_tests 8 "$caffe2_classification_path" "${output_path}Posterize_${rgb_name[$rgb]}_${device_name}" $width $height 79 $device $rgb 0 $display
        ./unit_tests 11 "$mxnet_path" "${output_path}Solarize_${rgb_name[$rgb]}_${device_name}" $width $height 81 $device $rgb 0 $display
